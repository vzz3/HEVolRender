...
TEST_CASE( "paillier FP signum", "[paillierMath]" ) {
	...
	REQUIRE( es32u.signum(EncodedNumber(SArbBigInt::fromInt64(0), 1)) == 0);
	REQUIRE( es32u.signum(EncodedNumber(SArbBigInt::fromInt64(1), 1)) == 1);
	REQUIRE( es32u.signum(EncodedNumber(sk32.publicKey.modulus - SArbBigInt(1), 1)) == 1);

	EncodingScheme es32s(sk32.publicKey, true,  8, 10);
	REQUIRE( es32s.signum(EncodedNumber(SArbBigInt::fromInt64(0), 1)) == 0);
	REQUIRE( es32s.signum(EncodedNumber(SArbBigInt::fromInt64(1), 1)) == 1);
	REQUIRE( es32s.signum(EncodedNumber(sk32.publicKey.modulus - SArbBigInt(1), 1)) == -1);
	REQUIRE( es32s.signum(EncodedNumber(sk32.publicKey.modulus / SArbBigInt(2) + SArbBigInt(1), 1)) == -1);
	REQUIRE( es32s.signum(EncodedNumber(sk32.publicKey.modulus / SArbBigInt(2) - SArbBigInt(1), 1)) == 1);
	REQUIRE( es32s.signum(EncodedNumber(sk32.publicKey.modulus / SArbBigInt(2), 1)) == 1);
}
...
TEST_CASE( "paillier FP encode integer", "[paillierMath]" ) {
	...
	REQUIRE( t1u.mantissa == SArbBigInt::fromInt64(0));
	REQUIRE( t1u.exponent == 0);

	EncodedNumber t2u = esU.encode(1);
	REQUIRE( t2u.mantissa == SArbBigInt::fromInt64(1));
	REQUIRE( t2u.exponent == 0);

	EncodedNumber t3u = esU.encode(987654321);
	REQUIRE( t3u.mantissa == SArbBigInt::fromInt64(987654321));
	...
	REQUIRE( t1s.mantissa == SArbBigInt::fromInt64(0));
	REQUIRE( t1s.exponent == 0);

	EncodedNumber t2s = esS.encode(1);
	REQUIRE( t2s.mantissa == SArbBigInt::fromInt64(1));
	REQUIRE( t2s.exponent == 0);

	EncodedNumber t3s = esS.encode(987654321);
	REQUIRE( t3s.mantissa == SArbBigInt::fromInt64(987654321));
	REQUIRE( t3s.exponent == 0);

	EncodedNumber t4s = esS.encode(-1);
	REQUIRE( t4s.mantissa == ( sk.publicKey.modulus - SArbBigInt(1)) );
	REQUIRE( t4s.exponent == 0);

	EncodedNumber t5s = esS.encode(-186574268);
	REQUIRE( t5s.mantissa == ( sk.publicKey.modulus - SArbBigInt::fromInt64(186574268)) );
	REQUIRE( t5s.exponent == 0);

	// test mantissa optimization
	EncodedNumber t10s = esS.encode(SArbBigInt::fromString("987654321000", 10));
	REQUIRE( t10s.mantissa == SArbBigInt::fromString("987654321", 10) );
	REQUIRE( t10s.exponent == 3);

	EncodedNumber t11s = esS.encode(345010000);
	REQUIRE( t11s.mantissa == SArbBigInt::fromInt64(34501) );
	REQUIRE( t11s.exponent == 4);

	// test big things
	SArbBigInt nineByteVal = SArbBigInt::fromString("-99 88 77 66 55 44 33 22 11", 16); // 9 Bytes
	...
	REQUIRE_NOTHROW( esU.encode(SArbBigInt::fromString("1", 2) << (modulusLen-1)) );
	REQUIRE_THROWS( esU.encode(SArbBigInt::fromString("1", 2) << modulusLen)) ;

	REQUIRE_THROWS( esS.encode(SArbBigInt::fromString("1", 2) << (modulusLen-1)) );
	REQUIRE_NOTHROW( esS.encode(SArbBigInt::fromString("1", 2) << (modulusLen-2)) );
	REQUIRE_THROWS( esS.encode(SArbBigInt::fromString("-1", 2) << (modulusLen-1)) );
	REQUIRE_NOTHROW( esS.encode(SArbBigInt::fromString("-1", 2) << (modulusLen-2)) );
}
...
TEST_CASE( "paillier FP encode float", "[paillierMath]" ) {
	...
	REQUIRE( t6s.mantissa == ( sk.publicKey.modulus - SArbBigInt::fromInt64(1865742689123)) );
	REQUIRE( t6s.exponent == -3);

	EncodedNumber t7s = esS.encode(-1865742689.123456, 0.0001);
	REQUIRE( t7s.mantissa == ( sk.publicKey.modulus - SArbBigInt::fromInt64(18657426891234)) );
	REQUIRE( t7s.exponent == -4);

	EncodedNumber t8s = esS.encode(-1865742689.123456, 0.00001);
	REQUIRE( t8s.mantissa == ( sk.publicKey.modulus - SArbBigInt::fromInt64(186574268912345)) );
	REQUIRE( t8s.exponent == -5);

	EncodedNumber t9s = esS.encode(-1865742689.123456, 0.000001);
	REQUIRE( t9s.mantissa == ( sk.publicKey.modulus - SArbBigInt::fromInt64(1865742689123456)) );
	REQUIRE( t9s.exponent == -6);

	EncodedNumber t10s = esS.encode(1865742689.123456, 0.000001);
	REQUIRE( t10s.mantissa == ( SArbBigInt::fromInt64(1865742689123456)) );
	...
	REQUIRE( t11s.mantissa != ( SArbBigInt::fromInt64(18657426891234567)) );
	REQUIRE( t11s.exponent == -7);

	static const size_t doubleExpBitLen = 11;
	static const size_t doubleManBitLen = 52;
	static const SArbBigInt doubleMaxMantisa = SArbBigInt(2).pow(SArbBigInt(doubleManBitLen + 1)); // = 9007199254740992
	...
	EncodedNumber t13s = esS.encode((double)(doubleMaxMantisa + SArbBigInt(1)).toInt64(), 1.0); // mantissa to large 9007199254740993
	REQUIRE( t13s.mantissa != doubleMaxMantisa + SArbBigInt(1));
...
TEST_CASE( "paillier FP decode integer", "[paillierMath]" ) {
	...
	REQUIRE( esS.decodeBigInt(esS.encode(1)) == SArbBigInt::fromString("1", 10) );
	REQUIRE( esS.decodeBigInt(esS.encode(-1)) == SArbBigInt::fromString("-1", 10) );
	REQUIRE( esS.decodeBigInt(esS.encode(-4564564568775367147)) == SArbBigInt::fromString("-4564564568775367147", 10) );

	SArbBigInt t1 = SArbBigInt::fromString("-54 68 24 45 24 34 50 46 57 61 56 48 96", 16);
	REQUIRE( esS.decodeBigInt(esS.encode(t1)) == t1 );
	REQUIRE( esS.decodeBigInt(esS.encode(t1*t1)) == t1 * t1 );

	SArbBigInt t2bigInt = SArbBigInt::fromString("-48 96 25 87 46 95 78 65 21 48 23 65 21 45", 16);
	EncodedNumber t2encoded = esS.encode(t2bigInt);
	int t2exp = 13;
	EncodedNumber t2encodedNew(t2encoded.mantissa, t2encoded.exponent + t2exp);
	REQUIRE( esS.decodeBigInt(t2encodedNew) == t2bigInt * (SArbBigInt::fromString("1" + std::string(t2exp, '0'), esS.getBase())) );
}
...
TEST_CASE( "paillier FP decode float", "[paillierMath]" ) {
	...
	static const size_t doubleExpBitLen = 11;
	static const size_t doubleManBitLen = 52;
	static const SArbBigInt doubleMaxMantissa = SArbBigInt(2).pow(SArbBigInt(doubleManBitLen + 1)); // = 9007199254740992
	//std::cout << doubleMaxMantisa.toStringDec() << std::endl;
	...
	REQUIRE( esSbas2.decodeDouble( esSbas2.encode(t2bigInt) ) == std::stod((t2bigInt + (SArbBigInt(1) << (t2bigInt.bitLength() - doubleManBitLen - 3)) ).toStringDec()) );
	// big error that not mether
	REQUIRE( esSbas2.decodeDouble( esSbas2.encode(t2bigInt) ) != std::stod((t2bigInt + (SArbBigInt(1) << (t2bigInt.bitLength() - doubleManBitLen + 1)) ).toStringDec()) );
...
TEST_CASE( "paillier FP paillier context encrypt/decrypt", "[paillierMath]" ) {
	...
	SArbBigInt sArbInt1 = SArbBigInt::fromString("-345464564126928411397", 10);
