...
EncodedNumber PaillierContext::decreaseExponentTo(const EncodedNumber& yEncodedNumber, const int32_t yNewExp) const {
	//SInfinitBigInt significand = encodedNumber.getValue();
	...
	const SInfinitBigInt bigFactor = encoder.getRescalingFactor(expDiff);
	const SInfinitBigInt newEnc = (yEncodedNumber.mantissa * bigFactor) % getPublicKey().modulus;
...
EncryptedNumber PaillierContext::decreaseExponentTo(const EncryptedNumber& yEncryptedNumber, const int32_t yNewExp) const {
	...
	const SInfinitBigInt bigFactor = encoder.getRescalingFactor(expDiff);
	const SInfinitBigInt newEnc = encoder.getPublicKey().multiply(yEncryptedNumber.ciphertext, bigFactor);
...
EncryptedNumber PaillierContext::encrypt(const EncodedNumber& yEncodedNumber, const bool yObfuscate) const {
	const SInfinitBigInt& mantissa = yEncodedNumber.mantissa;
	const SInfinitBigInt ciphertext = yObfuscate ? encoder.getPublicKey().encrypt(mantissa) : encoder.getPublicKey().encryptWithoutObfuscation(mantissa);
	return EncryptedNumber(ciphertext, yEncodedNumber.exponent);
}

EncryptedNumber PaillierContext::encrypt(const SInfinitBigInt& yValue, const bool yObfuscate) const {
...
EncryptedNumber PaillierContext::add(const EncryptedNumber& yOperand1, const EncryptedNumber& yOperand2) const {
	...
	SInfinitBigInt mantissa1 = yOperand1.ciphertext;
	SInfinitBigInt mantissa2 = yOperand2.ciphertext;
	...
	const SInfinitBigInt result = getPublicKey().add(mantissa1, mantissa2);
...
EncryptedNumber PaillierContext::add(const EncryptedNumber& yOperand1, const EncodedNumber& yOperand2) const {
	...
	int32_t exponent1 = yOperand1.exponent;
	int32_t exponent2 = yOperand2.exponent;
	SInfinitBigInt mantissa2 = yOperand2.mantissa;
	if (exponent1 < exponent2) {
...
EncodedNumber PaillierContext::add(const EncodedNumber& yOperand1, const EncodedNumber& yOperand2) const {
	...
	SInfinitBigInt mantissa1 = yOperand1.mantissa;
	SInfinitBigInt mantissa2 = yOperand2.mantissa;
	...
	const SInfinitBigInt result = (mantissa1 + mantissa2) % getPublicKey().modulus;
...
EncryptedNumber PaillierContext::multiply(const EncryptedNumber& yOperand1, const EncodedNumber& yOperand2) const {
	//checkSameContext(operand1);
	//checkSameContext(operand2);
	SInfinitBigInt encMantissa = yOperand1.ciphertext;
	SInfinitBigInt mantissa = yOperand2.mantissa;
	SInfinitBigInt negMantissa = getPublicKey().modulus - mantissa;
	...
	const SInfinitBigInt result = getPublicKey().multiply(encMantissa, mantissa);
...
EncodedNumber PaillierContext::multiply(const EncodedNumber& yOperand1, const EncodedNumber& yOperand2) const {
	//checkSameContext(operand1);
	//checkSameContext(operand2);
	const  SInfinitBigInt result = (yOperand1.mantissa * yOperand2.mantissa) % getPublicKey().modulus;
	const int32_t exponent = yOperand1.exponent + yOperand2.exponent;
